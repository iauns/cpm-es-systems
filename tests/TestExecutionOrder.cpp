...
static std::vector<std::string> execStack;

void addToExecutionStack(const std::string& name)
{
  if (execStack.size() == 0 || execStack.back() != name)
  {
    execStack.push_back(name);
  }
}
...
TEST(EntitySystem, TestExecutionOrder)
{
  ...
  systems->addActiveSystemViaType<C>();
  systems->addActiveSystemViaType<Alower>();
  systems->addActiveSystemViaType<SystemOne>();
  systems->addActiveSystemViaType<B>();
  systems->addActiveSystemViaType<BasicSystem>();
  systems->addActiveSystemViaType<A>();
  systems->renormalize();

  systems->runSystems(*core, 0);

  // Check the execution stack to ensure it is in the order we expect.
  ...
  int i = 0;
  for (const std::string& expected : expectedStack)
  {
    EXPECT_EQ(expected, execStack[i]);
    ++i;
  }
}
